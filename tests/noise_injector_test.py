import unittest
import unittest.mock
import io
import numpy as np
from PIL import Image

# add folder to path to make relative imports work
import sys,os
BASE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, BASE)
from augmentator.noise_injector import Noise_Injector


class TestNoiseInjector(unittest.TestCase):

    def test_default_values(self):
        ni = Noise_Injector()
        self.assertEqual(ni.method, 'gauss')
        self.assertEqual(ni.value, 0.5)


    @unittest.mock.patch('sys.stdout', new_callable=io.StringIO)
    def test_setter(self, mock_stdout):
        ni = Noise_Injector('salt_pepper', 0.4)
        self.assertEqual(ni.method, 'salt_pepper')
        self.assertEqual(ni.value, 0.4)

        ni.set_method('???')
        ni.set_value(3.0)
        self.assertEqual(ni.method, 'gauss')
        self.assertEqual(ni.value, 0.5)


    def test_image_dims(self):
        ni = Noise_Injector()

        img = np.array(Image.open('tests/images/butterfly.jpg'))
        img_orig_shape = img.shape
        img = ni.augment(img)

        self.assertEqual(img_orig_shape, img.shape)



### RUN
unittest.main()